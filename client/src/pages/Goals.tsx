...
import { Spinner, EmptyState, Button, Input, Modal, useToast, ConfirmDialog, PageHeader } from "@/components/ui";
...
export function Goals() {
  ...
  return (
    <div className="page-shell">
      <PageHeader
        title="Objetivos"
        lead="Metas con progreso visual"
        actions={<Button onClick={() => setCreateOpen(true)}><Plus className="w-4 h-4" />Nuevo objetivo</Button>}
      />
            ...
            return (
              <div key={g.id} className={clsx("card p-5", done && "opacity-70")}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="chip shrink-0 bg-ok/15 text-ok">{g.progress}%</span>
                </div>
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Nuevo objetivo"
